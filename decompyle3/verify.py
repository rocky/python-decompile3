...
import decompyle3
from decompyle3.scanner import Token as ScannerToken, get_scanner
        ...
        if member in __IGNORE_CODE_MEMBERS__ or verify != "verify":
            ...
            if flags1 != flags2:
                raise CmpErrorMember(
                    name, "co_flags", pretty_flags(flags1), pretty_flags(flags2)
                )
        else:
...
def compare_code_with_srcfile(pyc_filename, src_filename, verify):
    ...
    """
    (version, timestamp, magic_int, code_obj1, is_pypy, source_size) = load_module(
        pyc_filename
    )
...
def compare_files(pyc_filename1, pyc_filename2, verify):
    ...
    (
        ...
        is_pypy,
        source_size,
    ) = decompyle3.load_module(pyc_filename1)
    (
        version2,
        timestamp,
        magic_int2,
        ...
        is_pypy,
        source_size,
    ) = decompyle3.load_module(pyc_filename2)
